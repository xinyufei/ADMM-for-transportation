...
def Benders(epsilon = 0.01, num_scenario = 100):
    ub = np.infty
    lb = -np.infty
    ...
    while abs(ub-lb) > epsilon:
        ...
        for xi in range(num_scenario):
            ...
            if m_sub.status == 2:
                ...
                for i in range(N):
                    opt_sub[i,xi] = sum(sum(t * y_value[c+add[i],t] for c in D[i]) for t in range(T)) + alpha * sum(sum(t* y_value[c+add[i],t] for c in list(set(C[i])-set(D[i]))) for t in range(T))
...
if __name__ == '__main__':
    ...
    Benders()
    total_end = time.time()
    print("time to solve whole problem is %f" % (total_start - total_end))
    f = open('benders/T' + str(T) + '_S' + str(num_scenario) + '_bound_LP.log', 'a+')
    print("time to solve whole problem is %f" % (total_start - total_end), file = f)