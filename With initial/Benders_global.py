...
def Benders(epsilon = 0.01, num_scenario = 100, T = 20):
    ub = np.infty
    lb = -np.infty
    """ f = open('benders/T' + str(T) + '_S' + str(num_scenario) + '_bound_LP.log', 'w+')
    f = open('benders/T' + str(T) + '_S' + str(num_scenario) + '_w.log', 'w+')
    f = open('benders/T' + str(T) + '_S' + str(num_scenario) + '_y.log', 'w+')
    f = open('benders/T' + str(T) + '_S' + str(num_scenario) + '_n.log', 'w+') """
    ...
    xi = 0
    m_init = gb.Model()
    w = m_init.addVars(N, 4, T, lb=0, ub=1, vtype=gb.GRB.CONTINUOUS)
    new_w = m_init.addVars(N, T, lb=0, ub=1, vtype=gb.GRB.CONTINUOUS)
    u = m_init.addVars(N, T-1, lb=0, ub=1,  vtype=gb.GRB.CONTINUOUS)
    y = m_init.addVars(len(C_ALL), T, lb=0, vtype=gb.GRB.CONTINUOUS)
    n = m_init.addVars(len(C_ALL), T+1, lb=0, vtype=gb.GRB.CONTINUOUS)
    m_init.setObjective(-gb.quicksum(gb.quicksum((T - t) * y[c,t] for c in C_ALL) for t in range(T)) -
                    alpha*gb.quicksum(gb.quicksum(n[c,t] for c in D_ALL) for t in range(T)), gb.GRB.MINIMIZE)
    m_init.addConstrs(new_w[i,t] == w[i,0,t] + w[i,1,t] for i in range(N) for t in range(T))
    m_init.addConstrs(w[i,0,t]+w[i,1,t]+w[i,2,t]+w[i,3,t]==1 for i in range(N) for t in range(T))
    m_init.addConstrs(new_w[i,t]-new_w[i,t-1]-u[i,t-1]<=0 for i in range(N) for t in range(1,T))
    m_init.addConstrs( -new_w[i,t]+new_w[i,t-1]-u[i,t-1]<=0 for i in range(N) for t in range(1,T))
    m_init.addConstrs(-new_w[i,t]-new_w[i,t-1]+u[i,t-1]<=0 for i in range(N) for t in range(1,T))
    m_init.addConstrs(new_w[i,t]+new_w[i,t-1]+u[i,t-1]<=2 for  i in range(N) for t in range(1,T))
    m_init.addConstrs(u[i,t]+u[i,t+1]+u[i,t+2]+u[i,t+3]<=1 for  i in range(N) for t in range(0,T-4))
    m_init.addConstrs(new_w[i,t]+new_w[i,t+1]+new_w[i,t+2]+new_w[i,t+3]+new_w[i,t+4]+new_w[i,t+5]+new_w[i,t+6]+new_w[i,t+7] <= 8 for  i in range(N) for t in range(0,T-7))
    m_init.addConstrs(-(new_w[i,t]+new_w[i,t+1]+new_w[i,t+2]+new_w[i,t+3]+new_w[i,t+4]+new_w[i,t+5]+new_w[i,t+6]+new_w[i,t+7])<=-1 for  i in range(N) for t in range(0,T-7))
    m_init.addConstrs(y[c,t]-w[i,j,t]*Q_ALL[c]<=0 for i in range(N) for j in range(4) for c in I[i][j] for t in range(T))
    m_init.addConstrs(y[c,t]-n[c,t]<=0 for c in C_ALL for t in range(T))
    m_init.addConstrs(y[c,t]<=Q[0][0] for c in list(set(C_ALL)-set(I1_ALL)-set(I2_ALL)-set(I3_ALL)-set(I4_ALL)-set(D_ALL)) for t in range(T))
    for i in range(N):
        m_init.addConstrs(y[c+add[i],t]<=Q[i][c+1]/beta[i][xi][t][0,0] for c in V[i] for t in range(T))
        m_init.addConstrs(y[c+add[i],t]<=Q[i][c+2]/beta[i][xi][t][1,0] for c in V[i] for t in range(T))
        m_init.addConstrs(y[c+add[i],t]<=Q[i][c+3]/beta[i][xi][t][2,0] for c in V[i] for t in range(T))
    m_init.addConstrs(y[c,t]+W*n[proc_all[c],t]<=W*Jam_N_ALL[c] for c in list(set(C_ALL)-set(D_ALL)-set(V_ALL)) for t in range(T))
    for i in range(N):
        m_init.addConstrs(beta[i][xi][t][0,0]*y[c+add[i],t]+W*n[c+add[i]+1,t]<=W*Jam_N_ALL[int(c+add[i]+1)] for c in V[i] for t in range(T))
        m_init.addConstrs(beta[i][xi][t][1,0]*y[c+add[i],t]+W*n[c+add[i]+2,t]<=W*Jam_N_ALL[int(c+add[i]+2)] for c in V[i] for t in range(T))
        m_init.addConstrs(beta[i][xi][t][2,0]*y[c+add[i],t]+W*n[c+add[i]+3,t]<=W*Jam_N_ALL[int(c+add[i]+3)] for c in V[i] for t in range(T))
        m_init.addConstrs(n[c+add[i],t+1] - n[c+add[i],t] - beta[i][xi][t][c-pred[i][c]-1,0]*y[pred[i][c]+add[i],t] + y[c+add[i],t] == 0 for c in I1[i]+I2[i]+I3[i]+I4[i] for t in range(T))
    m_init.addConstrs(n[c,t+1] - n[c,t] - y[pred_all[c],t] + y[c,t] == 0 for c in list(set(C_ALL)-set(O_ALL)-set(I1_ALL)-set(I2_ALL)-set(I3_ALL)-set(I4_ALL)-set(M_ALL)) for t in range(T))
    m_init.addConstrs(n[c,t+1] - n[c,t] - gb.quicksum(y[d,t] for d in pred_all[c]) + y[c,t] == 0 for c in M_ALL for t in range(T))
    m_init.addConstrs(n[O_ALL[i],t+1] - n[O_ALL[i],t] + y[O_ALL[i],t] == Demand_ALL[i][xi][t] for i in range(len(O_ALL)) for t in range(T))
    m_init.addConstrs((n[c,0] == n_init_all[c] for c in C_ALL))
    m_init.optimize()
    w_tilde = np.zeros((N,4,T))
    w_value = m_init.getAttr('X',w)
    for i in range(N):
        for j in range(4):
            for t in range(T):
                if w_value[i,j,t] < 0.5:
                    w_tilde[i,j,t] = 0
                if w_value[i,j,t] >= 0.5:
                    w_tilde[i,j,t] = 1
    ...
    while (ub-lb)/abs(lb) > epsilon or lb == -np.infty:
        ...
        f = open('benders/T' + str(T) + '_S' + str(num_scenario) + '_bound_LP.log', 'a+')
        ...
        f = open('benders/T' + str(T) + '_S' + str(num_scenario) + '_w.log', 'a+')
        print("iteration " + str(num_ite), file = f)
        for i in range(N):
            for j in range(4):
                for t in range(T):
    ...
    f = open('benders/T' + str(T) + '_S' + str(num_scenario) + '_n.log', 'a+')
    for i in range(N):
        for t in range(T+1):
            print(n_value[i,t], end = ",", file=f)
        print("\n", file=f)
    f = open('benders/T' + str(T) + '_S' + str(num_scenario) + '_y.log', 'a+')
    for i in range(N):
        for t in range(T):
