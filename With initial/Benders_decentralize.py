...
def Benders(epsilon = 0.01, num_scenario = 100, T = 20):
    ...
    w_tilde = [None]*N
    theta_tilde = [None]*N
    n_value = m_init.getAttr('X',n)
    y_value = m_init.getAttr('X',y)
    for i in range(N):
        theta_tilde[i] = np.ones(num_scenario)*(-sum(sum((T - t) * y_value[c+add[i],t] for c in C[i]) for t in range(T)) -
            alpha*sum(sum(n_value[c+add[i],t] for c in D[i]) for t in range(T)))
    ...
    print("the objective value is %f" % (sum(eval_delay)/num_scenario))
    print("the ctm objective value is %f" % (sum(eval_ctm)/num_scenario))
    f = open('benders_decentralize/T' + str(T) + '_S' + str(num_scenario) + '_bound_LP.log', 'a+')
    print("the objective value is %f" % (sum(eval_delay)/num_scenario), file = f)
