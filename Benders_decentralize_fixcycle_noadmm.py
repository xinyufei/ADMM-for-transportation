...
def Benders(epsilon = 0.01, num_scenario = 100, T = 20):
    ub = np.infty
    lb = -np.infty
    f = open('H_4/T' + str(T) + '_S' + str(num_scenario) + '_bound_LP_fixed_length_decentralize.log', 'w+')
    f = open('H_4/T' + str(T) + '_S' + str(num_scenario) + '_w_fixed_length_decentralize.log', 'w+')
    f = open('H_4/T' + str(T) + '_S' + str(num_scenario) + '_y_fixed_length_decentralize.log', 'w+')
    f = open('H_4/T' + str(T) + '_S' + str(num_scenario) + '_n_fixed_length_decentralize.log', 'w+')
    f = open('debug.txt', 'w+')
   ...
   # build subproblem
    ...
    for xi in range(num_scenario):
        ...
        cons3[xi] = m_sub[xi].addConstrs(y[xi][c+add[i],t]<=Q[i][0] for i in range(N) for c in list(set(C[i])-set(I1[i])-set(I2[i])-set(I3[i])-set(I4[i])-set(D[i])) for t in range(T))
        cons4[xi] = [None]*3*N
        for i in range(N):
        ...
        m_sub[xi].Params.LogFile = 'H_4/T' + str(T) + '_S' + str(num_scenario) + '_sub.log'
    ...
    while 1:
        ...
        while 1:
            ...
            for i in range(N):
                ...
                print("\n", file=f)
            f = open('H_4/T' + str(T) + '_S' + str(num_scenario) + '_binary_fixed_length_decentralize.log', 'a+')
        ...
        """ if ub_all_array >= 3:
            if abs(ub_all_array[-1] - ub_all_array[-2]) < 0.01*abs(ub_all_array[-1]) and abs(ub_all_array[-3] - ub_all_array[-2]) < 0.01*abs(ub_all_array[-2]):
                break """
    f = open('H_4/T' + str(T) + '_S' + str(num_scenario) + '_optimal_signal_fixed_length_decentralize.log', 'a+')
    for i in range(N):
        for p in range(4):
    ...
    plt.legend()
    plt.savefig('H_4/T' + str(T) + '_S' + str(num_scenario) + '_obj_fixed_length_decentralize.jpg')
    ...
    plt.legend()
    plt.savefig('H_4/T' + str(T) + '_S' + str(num_scenario) + '_ub_fixed_length_decentralize.jpg')
    ...
    plt.legend()
    plt.savefig('H_4/T' + str(T) + '_S' + str(num_scenario) + '_lb_fixed_length_decentralize.jpg')
...
if __name__ == '__main__':
    ...
    f3 = open('H_4/T' + str(T) + '_S' + str(num_scenario) + '_lb_fixed_length_decentralize.txt', 'w+')
