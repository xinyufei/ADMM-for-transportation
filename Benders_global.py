...
def Benders(epsilon = 0.01, num_scenario = 100, T = 20):
    ub = np.infty
    lb = -np.infty
    f = open('benders/T' + str(T) + '_S' + str(num_scenario) + '_bound_LP_variant_length.log', 'w+')
    f = open('benders/T' + str(T) + '_S' + str(num_scenario) + '_w_variant_length.log', 'w+')
    f = open('benders/T' + str(T) + '_S' + str(num_scenario) + '_y_variant_length.log', 'w+')
    f = open('benders/T' + str(T) + '_S' + str(num_scenario) + '_n_variant_length.log', 'w+')
    ...
    print(T)
    m_master = gb.Model()
    z1 = m_master.addVars(N, 4, num_cycle, T, vtype=gb.GRB.BINARY)
    z2 = m_master.addVars(N, 4, num_cycle, T, vtype=gb.GRB.BINARY)
    # z = m_master.addVars(4, T, vtype=gb.GRB.BINARY)
    e = m_master.addVars(N, 4, num_cycle, lb = -np.infty)
    b = m_master.addVars(N, 4, num_cycle, lb = -np.infty)
    g = m_master.addVars(N, 4, lb = 2, ub = 25)
    l  = m_master.addVars(N)
    ...
    m_master.update()
    m_master.addConstrs(-U*z1[i,p,cy,t]+epsilon <= t-e[i,p,cy] for i in range(N) for p in range(4) for t in range(T) for cy in range(num_cycle))
    m_master.addConstrs(-U*z2[i,p,cy,t] <= b[i,p,cy]-t for i in range(N) for p in range(4) for t in range(T) for cy in range(num_cycle))
    m_master.addConstrs(t-e[i,p,cy] <= U*(1-z1[i,p,cy,t]) for i in range(N) for p in range(4) for t in range(T) for cy in range(num_cycle))
    m_master.addConstrs(b[i,p,cy]-t <= U*(1-z2[i,p,cy,t])-epsilon for i in range(N) for p in range(4) for t in range(T) for cy in range(num_cycle))
    # m_master.addConstrs(z1[p,t]+z2[p,t]-z[p,t] == 1 for p in range(4) for t in range(T))
    m_master.addConstrs(gb.quicksum(z1[i,p,cy,t]+z2[i,p,cy,t] for p in range(4)) <= 5 for i in range(N) for cy in range(num_cycle) for t in range(T))
    m_master.addConstrs(o[i] <= l[i] for i in range(N))
    m_master.addConstrs(b[i,0,cy] == l[i]*cy - o[i] for i in range(N) for cy in range(num_cycle))
    m_master.addConstrs(e[i,0,cy] == b[i,0,cy] + g[i,0] for i in range(N) for cy in range(num_cycle))
    m_master.addConstrs(b[i,1,cy] == e[i,0,cy] for i in range(N) for cy in range(num_cycle))
    m_master.addConstrs(e[i,1,cy] == b[i,1,cy] + g[i,1] for i in range(N) for cy in range(num_cycle))
    m_master.addConstrs(b[i,2,cy] == e[i,1,cy] for i in range(N) for cy in range(num_cycle))
    m_master.addConstrs(e[i,2,cy] == b[i,2,cy] + g[i,2] for i in range(N) for cy in range(num_cycle))
    m_master.addConstrs(b[i,3,cy] == e[i,2,cy] for i in range(N) for cy in range(num_cycle))
    m_master.addConstrs(e[i,3,cy] == b[i,3,cy] + g[i,3] for i in range(N) for cy in range(num_cycle))
    m_master.addConstrs(gb.quicksum(g[i,p] for p in range(4)) - l[i] == 0 for i in range(N))
    ...
    for xi in range(num_scenario):
        ...
        cons1[xi] = m_sub[xi].addConstrs(y[xi][c,t]-gb.quicksum(z1_tilde[i,p,cy,t]+z2_tilde[i,p,cy,t]-1 for cy in range(num_cycle))*Q_ALL[c] <= 0 for i in range(N) for p in range(4) for c in I[i][p] for t in range(T))
    ...
    while 1:
        ...
        for xi in range(num_scenario):
            ...
            if m_sub[xi].status == 2:
                ...
                constant = 0
                scope = 0
                scope = gb.quicksum(-gb.quicksum(z1[i,p,cy,t]+z2[i,p,cy,t] for cy in range(num_cycle))*Q_ALL[c]*cons1[xi][i,p,c,t].Pi for i in range(N) for p in range(4) for c in I[i][p] for t in range(T))
                constant = constant - gb.quicksum(Q_ALL[c]*num_cycle*cons1[xi][i,p,c,t].Pi for i in range(N) for p in range(4) for c in I[i][p] for t in range(T))
        ...
        plot_vehicle(num_ite, T, num_scenario, "variant_length", add, len(C_ALL), n_value_average, y_value_average)
        end_plot = time.time()
        ...
        f = open('benders/T' + str(T) + '_S' + str(num_scenario) + '_bound_LP_variant_length.log', 'a+')
        ...
        f.close()
        f = open('benders/T' + str(T) + '_S' + str(num_scenario) + '_signal_variant_length.log', 'a+')
        print("iteration " + str(num_ite), file = f)
        for i in range(N):
            for p in range(4):
    ...
    for xi in range(num_scenario):
        ...
        m_eval.addConstrs(y[c,t]-gb.quicksum(z1_optimal[i,p,cy,t]+z2_optimal[i,p,cy,t]-1 for cy in range(num_cycle))*Q_ALL[c]<=0 for i in range(N) for p in range(4) for c in I[i][p] for t in range(T))
        ...
        f = open('benders/T' + str(T) + '_S' + str(num_scenario) + '_n_variant_length.log', 'a+')
        for c in C_ALL:
            for t in range(T+1):
                print(n_value[i,t], end = ",", file=f)
            print("\n", file=f)
        f = open('benders/T' + str(T) + '_S' + str(num_scenario) + '_y_variant_length.log', 'a+')
        for c in C_ALL:
            for t in range(T):
    ...
    plt.legend()
    plt.savefig('benders/T' + str(T) + '_S' + str(num_scenario) + '_obj_variant_length.jpg')
    ...
    plt.legend()
    plt.savefig('benders/T' + str(T) + '_S' + str(num_scenario) + '_ub_variant_length.jpg')
    ...
    plt.legend()
    plt.savefig('benders/T' + str(T) + '_S' + str(num_scenario) + '_lb_variant_length.jpg')

    return obj_array, ub_array, lb_array
    
if __name__ == '__main__':
    # Benders(0.0001, 10, 20)
    # Benders(0.0001, 100, 50)
    # Benders(0.0001, 100, 100)
    T = 50
    num_scenario = 10
    obj_array, ub_array, lb_array = Benders(0.0001, 10, T)
    f1 = open('benders/T' + str(T) + '_S' + str(num_scenario) + '_obj_variant_length.txt', 'w+')
    f2 = open('benders/T' + str(T) + '_S' + str(num_scenario) + '_ub_variant_length.txt', 'w+')
    f3 = open('benders/T' + str(T) + '_S' + str(num_scenario) + '_lb_variant_length.txt', 'w+')
